...
saveBtn.addEventListener('click', async ()=>{
  ...
  try{
    ...
    if(needNewDoc){
      ...
      if(window.__stripMeta){ try{ finalDoc.setTitle(''); finalDoc.setAuthor(''); finalDoc.setSubject(''); finalDoc.setKeywords([]); finalDoc.setCreator(''); finalDoc.setProducer('PDF Direct Editor') }catch{} window.__stripMeta=false }
      else { try{ finalDoc.setAuthor('Otim Noah'); finalDoc.setProducer('PDF Direct Editor by Otim Noah'); finalDoc.setCreator('PDF Direct Editor v1.0 - Otim Noah') }catch{} }
      finalBytes = await finalDoc.save()
    } else {
      if(window.__stripMeta){ try{ pdfLibDoc.setTitle(''); pdfLibDoc.setAuthor(''); pdfLibDoc.setSubject(''); pdfLibDoc.setKeywords([]); pdfLibDoc.setCreator(''); pdfLibDoc.setProducer('PDF Direct Editor') }catch{} window.__stripMeta=false }
      else { try{ pdfLibDoc.setAuthor('Otim Noah'); pdfLibDoc.setProducer('PDF Direct Editor by Otim Noah') }catch{} }
